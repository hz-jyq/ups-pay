...
public interface CollectChooseDubboDao extends JpaRepository<CollectChooseEntity, Long>{
	...
	@Query(nativeQuery=true,value="SELECT * FROM ups_t_collect_choose WHERE IF(:#{#form.productId}!='',product_id=:#{#form.productId},1=1) AND IF(:#{#form.collectType}!='',collect_type=:#{#form.collectType},1=1) ORDER BY create_time DESC")
