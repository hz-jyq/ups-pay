 ...
 *
 */
@Service(group = "yeepayBind",timeout=60000,retries=0)
public class LakalaBindCardServiceImpl extends SignatureBindBaseCore implements BindCardService {
	...
	private Logger logger = LoggerFactory.getLogger(LakalaBindCardServiceImpl.class);
	...
	public UpsResultModel bindCard(UpsBindCardParamModel upsBindCardParamModel) throws BussinessException{
		...
		String configStr = upsThirdpartyConfigService.queryThirdpartyConfig(upsBindCardParamModel.getPayChannel(),
				upsBindCardParamModel.getOrderType(), upsBindCardParamModel.getProductId()).getConfigDate();
