package com.pgy.ups.pay.lakala.pay.impl;
...
public class LakalaSignatureServiceImpl extends SignatureBindBaseCore implements SignatureService {
	...
	private Logger logger = LoggerFactory.getLogger(LakalaSignatureServiceImpl.class);
	...
	public UpsResultModel signature(UpsSignatureParamModel upsSignatureParamModel) throws BussinessException{
		...
		String configStr = upsThirdpartyConfigService.queryThirdpartyConfig(upsSignatureParamModel.getPayChannel(),
				upsSignatureParamModel.getOrderType(), upsSignatureParamModel.getProductId()).getConfigDate();
			...
			if("SUCCESS".equals(yopresponse.getState())){
			 ...
			 if("BIND_SUCCESS".equals(status)){
				 if (upsAuthSignEntity == null) {
					 ...
					 upsAuthSignEntity.setProductId(upsSignatureParamModel.getProductId());
