...
public class MerchantOrderTypeServiceImpl implements MerchantOrderTypeService, Cacheable<MerchantOrderTypeEntity> {
	...
	// 查询商户支付产品对象
	@Override
	public MerchantOrderTypeEntity confirmMerchantOrderType(Long merchanCode, String orderType) {
		String key=CacheUtils.generateKey(String.valueOf(merchanCode), orderType);
		...
		if(mot == null){
			throw new BussinessException("配置文件为空");
		}
	...
	public Map<String, MerchantOrderTypeEntity> getCacheableData() {
		...
		if (CollectionUtils.isNotEmpty(list)) {
			for (MerchantOrderTypeEntity mot : list) {
				cacheMap.put(CacheUtils.generateKey(String.valueOf(mot.getMerchantConfigEntity().getProductId()),
