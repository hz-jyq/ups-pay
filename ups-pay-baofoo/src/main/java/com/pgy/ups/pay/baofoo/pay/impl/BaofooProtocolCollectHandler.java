...
public class BaofooProtocolCollectHandler implements BussinessHandler<UpsOrderEntity, UpsResultModel> {
	
	
	private Logger logger = LoggerFactory.getLogger(BaofooProtocolCollectHandler.class);
	...
	@Resource
	private BaofooProtocolCollectHttpRemoteInvoke baofooProtocolCollectHttpRemoteInvoke;
	...
	public UpsResultModel handler(UpsOrderEntity orderEntity) throws BussinessException {
		...
		// 查询用户协议信息
		UpsAuthSignEntity uasbe = upsAuthSignBaofooService.queryProtocolSignBaofoo(orderEntity.getFromSystem(),
		...
		baofooProtocolCollectHttpRemoteInvoke.httpRemoteInvoke(request_url,postData,public_key,orderEntity);
...
@Component
class BaofooProtocolCollectHttpRemoteInvoke{
	
	private Logger logger = LoggerFactory.getLogger(BaofooProtocolCollectHttpRemoteInvoke.class);
