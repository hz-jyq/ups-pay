...
public class YeepaySignatureHandler extends SignatureBindBaseCore implements BussinessHandler<UpsSignatureParamModel, UpsResultModel> {
	...
	private Logger logger = LoggerFactory.getLogger(YeepaySignatureHandler.class);

	@Override
	public UpsResultModel handler(UpsSignatureParamModel upsSignatureParamModel) throws BussinessException{
		...
		String configStr = upsThirdpartyConfigService.queryThirdpartyConfig(upsSignatureParamModel.getPayChannel(),
				upsSignatureParamModel.getOrderType(), upsSignatureParamModel.getFromSystem()).getConfigDate();
			...
			if("SUCCESS".equals(yopresponse.getState())){
			 ...
			 if("BIND_SUCCESS".equals(status)){
				 if (upsAuthSignEntity == null) {
					 ...
					 upsAuthSignEntity.setFromSystem(upsSignatureParamModel.getFromSystem());
