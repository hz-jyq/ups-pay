package com.pgy.ups.pay.route.worker.impl;
...
import com.pgy.ups.pay.route.intercepter.impl.RouteChooseWorkerIntercepter;
...
public class RouteChooseWorker implements Worker<String> {
	...
	public void initRouteChooseHandler(UpsParamModel upsParamModel) {
		...
		payChannels = payCompanyService.queryMerchantAvailableChannels(upsParamModel.getFromSystem(),
				upsParamModel.getOrderType());
	...
	public void doWorker() {
		while (intercepterIterator.hasNext()) {
			...
			if (StringUtils.isEmpty(payChannel) && CollectionUtils.isEmpty(payChannels)) {
				...
				payChannel = RouteUtils.getDefaultRouteResult(upsParamModel.getFromSystem());
			}
		}
	}
	...
	public List<PayCompanyEntity> getPayChannels() {
		return payChannels;
	}
